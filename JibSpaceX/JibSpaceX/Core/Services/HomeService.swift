...
class HomeService: HomeServiceProtocol {
    ...
    func getHomeItems(completion: @escaping (Dictionary<String, Any>?, Error?) -> Void) {
        ...
        //  The api doesn’t specify what parameter to use for the pagination but it should be implemented if available, and it's advised to have pagination in cases of lots of entries (would need filtering as well in that case since not all the data would be available for local filtering)
